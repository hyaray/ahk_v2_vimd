...
;-----------------------------------general__-----------------------------------
;轮流发送字符串+slepp(遇到数字)+按键
;send("{1000}") 相当于 sleep(1000)
sendEx(arr*) {
    for v in arr {
        if (v is integer) {
            sleep(v)
...
hyf_getSelect(bVimNormal:=false, bInput:=false) {
    ...
    if ClipWait(0.2) {
        ...
        if (bInput) {
            ...
            if (res.result=="Cancel" || !strlen(res.value))
                return
            return res.value
        }
    }
    ...
    if (type(var) == "String") {
        ...
        if !var.isabs() {
            if (var ~= "^\w+\(\S*\)$") { ;运行function()
                ...
                (m[3] != "") ?  %m[1]%.%m[2]%(m[3]) : %m[1]%.%m[2]%() ;NOTE 加 .call 有问题
                return 1
            }
...
hyf_runByVim(fp, line:=0, params:="--remote-tab-silent") { ;用文本编辑器打开
    ...
    OutputDebug(format('d:\TC\soft\Vim\gvim.exe {1} "{2}"', params, fp))
...
hyf_average(arr*) {
    ...
    for v in arr
        res += v
    return hyf_delete0(round(res/arr.length, 2))
}
...
hyjson2arr(obj, mark:=1) {
    arr := []
    for k, v in obj {
        if !strlen(v["key"]) ;首先过滤key为空的 NOTE
            continue
